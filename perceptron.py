    ...
    for shuffle in r: #iterates over r
        ...
        for algorithm in ['Perceptron', 'MLP']: #iterates over the algorithms
            ...
            if algorithm == 'Perceptron':
                clf = Perceptron(eta0=eta, shuffle=shuffle, max_iter=1000)    #use those hyperparameters: eta0 = learning rate, shuffle = shuffle the training data, max_iter=1000
            else:
               clf = MLPClassifier(activation = 'logistic', learning_rate_init=eta, hidden_layer_sizes=(25),shuffle = shuffle, max_iter=1000) #use those hyperparameters: activation='logistic', learning_rate_init = learning rate,
            ...
            if algorithm == 'Perceptron' and accuracy > best_perceptron_acc:
                ...
                print(f"Highest Perceptron accuracy so far: {accuracy:.2f}, Parameters: learning rate={eta}, shuffle={shuffle}")
            elif algorithm == 'MLP' and accuracy > best_mlp_acc:
                best_mlp_acc = accuracy
                print(f"Highest MLP accuracy so far: {accuracy:.2f}, Parameters: learning rate={eta}, shuffle={shuffle}")
